...
export default function License(){
    ...
    return (
        <div className={'p-10'}>
            <span className="block tracking-widest uppercase text-xs">Company</span>

            <div className="mt-10 flex items-center space-x-5">
                <span className="block text-3xl font-bold">Company</span>
                            ...
                            <tbody>
                            {
                                data.map((x:any,i:number)=>(
                                    <tr key={i} className={`${i%2===0 ? 'bg-zinc-800' : ''} hover:bg-zinc-950`}>
                                        <td onClick={()=>navigate(`/company/${x.id}`)} className={'text-xs cursor-pointer px-3 py-4 text-zinc-400'}>{x.name}</td>
